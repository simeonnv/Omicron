...
pub async fn get_post_req(subicron_id: i64, post_id: i64) -> Result<PostStruct, String> {
    ...
    let url = Url::parse(&format!("{}/subicron/{}/posts/{}", config::BACKEND_URL, subicron_id, post_id))
    ...
    if response.status().is_success() {
        let response: GetPostResponse = response.json().await.map_err(|e| e.to_string())?;
