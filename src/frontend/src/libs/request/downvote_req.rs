...
struct PostDownvoteResponse {
    status: String,
    data: String
}

pub async fn downvote_req(subicron_id: i64, post_id: i64) -> Result<String, String> {
    ...
    if response.status().is_success() {
        let response: PostDownvoteResponse = response.json().await.map_err(|e| e.to_string())?;
