...
use crate::{libs::request::signup_req::signup_req, router::Route, ui::{button::Button, input::Input}};
    ...
    let on_submit = {
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                match signup_req(username, password).await {
                    Ok(token) => {
        ...
        <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
            <div class="flex flex-col align-middle justify-center items-center">
                <div class="flex flex-col align-middle justify-center items-center gap-6 border-4 border-dashed border-purple-600 p-10 rounded-xl">
                    <h1 class="text-2xl pb-4">{ "Signup" }</h1>
                    ...
                    <div class="flex flex-col gap-1">
                        <p class="text-xs text-purple-500">{"Have an account?"}</p>
